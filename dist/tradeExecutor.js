...
class TradeExecutor {
    constructor(apiKey, isDemo, cst, securityToken) {
        this.apiKey = apiKey;
        this.isDemo = isDemo;
        this.cst = cst;
    ...
    async getOpenPositions() {
        try {
            ...
            logger_1.logger.error('Error fetching open positions:', err);
            return [];
        }
    }
        ...
        try {
            ...
            for (const activity of activities) {
                if (activity.type === 'POSITION' && activity.status === 'ACCEPTED') {
                    const epic = activity.details?.epic || activity.epic;
                    const closeDate = activity.date;
                    if (epic && closeDate && activity.details?.actions) {
                        const isClosed = activity.details.actions.some((a) => a.actionType === 'POSITION_CLOSED' || a.actionType === 'POSITION_DELETED');
                        if (isClosed) {
                            ...
                            if (!existing || closeTime > existing) {
                                ...
                                logger_1.logger.info(`Cooldown set for ${epic} — closed at ${closeDate}`);
                            }
                        }
                    }
                }
            ...
            logger_1.logger.error('Error syncing closed positions:', err);
        }
    }
    ...
    async openTrade(signal) {
        ...
        try {
            ...
            logger_1.logger.error(`Failed to open trade for ${signal.symbol}:`, errorMsg);

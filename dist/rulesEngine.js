...
exports.getMaxTrades = getMaxTrades;
exports.isBlockedByRules = isBlockedByRules;
...
function parseRule(raw) {
    ...
    if (timeMatch) {
        ...
        if (t)
            return { raw, type: 'TIME_BLOCK', blockHour: t.h, blockMinute: t.m };
    }
...
function loadRules() {
    ...
    if (!fs.existsSync(filePath)) {
        logger_1.logger.warn('rules.txt not found — no trading rules loaded.');
        rules = [];
        return;
    }
    ...
    for (const line of lines) {
        ...
        if (trimmed.startsWith('-') || trimmed.startsWith('#')) {
            ...
            if (parsed.type !== 'UNKNOWN') {
                logger_1.logger.info(`Rule loaded: [${parsed.type}] ${ruleText}`);
            }
            else {
                logger_1.logger.warn(`Rule nicht erkannt: ${ruleText}`);
            }
        }
    }
    logger_1.logger.info(`${rules.length} rules loaded from rules.txt`);
}
function getMaxTrades() {
    const rule = rules.find(r => r.type === 'MAX_TRADES');
    return rule?.maxTrades ?? 99;
}
function isBlockedByRules(nowMEZ) {

...
export function ZoneTable({ zones }: { zones: Record<string, Zone[]> }) {
  ...
  return (
    <div style={{ background: "#161b22", borderRadius: 8, padding: 12 }}>
      <h3
        onClick={() => setOpen((o) => !o)}
        style={{ margin: open ? "4px 0 12px" : "4px 0", cursor: "pointer",
                  userSelect: "none", display: "flex", alignItems: "center", gap: 6 }}
      >
        <span style={{ color: "#8b949e", fontSize: 12, transform: open ? "rotate(90deg)" : "none",
                        display: "inline-block", transition: "transform 0.15s" }}>▶</span>
        Areas of Interest
        <span style={{ color: "#8b949e", fontWeight: 400, fontSize: 13 }}>({rows.length})</span>
      </h3>
      {open && (

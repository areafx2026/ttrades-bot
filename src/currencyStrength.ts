...
// All pairs needed to calculate strength for 8 currencies
// Using pairs where Capital.com epic = standard Forex notation
...
export interface StrengthResult {
  scores: Record<string, number>;   // currency -> score
  ranked: string[];                  // currencies sorted strongest to weakest
  strong: string[];                  // top 2
  weak: string[];                    // bottom 2
  timestamp: Date;
}
...
export async function getCurrencyStrength(capital: CapitalAPI): Promise<StrengthResult> {
  ...
  for (const pair of STRENGTH_PAIRS) {
    try {
      const candles = await capital.getCandles(pair.epic, 'HOUR', 24);
      ...
      const change = (close - open) / pip; // in pips

      // Base currency gains if price goes up
      scores[pair.base]  += change;
      // Quote currency loses if price goes up
  ...
  // Normalize: divide by number of pairs each currency appeared in
...
// Check if a signal's pair aligns with currency strength
// Returns true if base is strong AND quote is weak (or vice versa for SHORT)
export function isStrengthAligned(
  symbol: string,
  direction: 'LONG' | 'SHORT',
  strength: StrengthResult
): { aligned: boolean; reason: string } {
  // Parse base and quote from symbol (e.g. GBPAUD -> GBP, AUD)
  ...
  if (direction === 'LONG') {
    // For LONG: base should be strong (top 2), quote should be weak (bottom 2)
    const baseStrong  = strength.strong.includes(base);
    const quoteWeak   = strength.weak.includes(quote);

    if (baseStrong && quoteWeak) {
      return { aligned: true, reason: `${base} stark (#${baseRank + 1}) + ${quote} schwach (#${quoteRank + 1})` };
    }
    if (baseStrong) {
      return { aligned: false, reason: `${base} stark aber ${quote} nicht schwach genug (#${quoteRank + 1})` };
    }
    if (quoteWeak) {
      return { aligned: false, reason: `${quote} schwach aber ${base} nicht stark genug (#${baseRank + 1})` };
    }
    return { aligned: false, reason: `${base} (#${baseRank + 1}) nicht stark, ${quote} (#${quoteRank + 1}) nicht schwach` };
  } else {
    // For SHORT: base should be weak (bottom 2), quote should be strong (top 2)
    const baseWeak    = strength.weak.includes(base);
    const quoteStrong = strength.strong.includes(quote);

    if (baseWeak && quoteStrong) {
      return { aligned: true, reason: `${base} schwach (#${baseRank + 1}) + ${quote} stark (#${quoteRank + 1})` };
    }
    if (baseWeak) {
      return { aligned: false, reason: `${base} schwach aber ${quote} nicht stark genug (#${quoteRank + 1})` };
    }
    if (quoteStrong) {
      return { aligned: false, reason: `${quote} stark aber ${base} nicht schwach genug (#${baseRank + 1})` };
    }
